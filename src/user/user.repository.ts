...
export class UserRepository {
    ...
    if (categories) {
      ...
      if (professionArray.length > 0) {
        filteredUsers = filteredUsers.filter((user) =>
          user.categories.some((category) => professionArray.includes(category.name.toLocaleLowerCase()))
        );
      }
    }
  ...
  async getClients(page: number, limit: number) {
    ...
    const usersMapped = users.map(
      ({ password, hashedRefreshToken, is_active, rating, services, availability, bio, portfolio_gallery, years_experience, ...user }) => {
